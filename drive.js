...
module.exports=async function handler(req,res){
  try{
    ...
    if(action==="saveMasterDataInit"){
      ...
      const manifest={
        schemaVersion:3.2,status:"uploading",uploadId:body.uploadId||null,
        market,department,savedAt:body.savedAt||new Date().toISOString(),
        totalRecords:Number(body.totalRecords)||0,totalChunks:Number(body.totalChunks)||0
      };
    ...
    if(action==="saveMasterDataChunk"){
      ...
      await upsertJson(drive,folder.id,`master_part_${String(idx).padStart(4,"0")}.json`,{
        schemaVersion:3.2,uploadId:body.uploadId||null,chunkIndex:idx,records
      });
    ...
    if(action==="saveMasterDataFinalize"){
      ...
      if(parts.files.length!==Number(manifest.totalChunks||0)){
        return res.status(409).json({error:`Stammdaten unvollständig: ${parts.files.length} von ${manifest.totalChunks} Blöcken vorhanden.`});
      }
      const payload={schemaVersion:3.2,market,department,savedAt:manifest.savedAt||new Date().toISOString(),records:parts.records};
      ...
      return res.status(200).json({ok:true,records:parts.records.length,path:`${root.name}/Stammdaten/${market}/${department}/masterdata.json`});
    }

    if(action==="saveMasterData"){
      const folder=await masterFolder(drive,root,market,department,true);
      const records=(Array.isArray(body.records)?body.records:[]).map(r=>({...r,department}));
      const payload={schemaVersion:3.2,market,department,savedAt:body.savedAt||new Date().toISOString(),records};
      await upsertJson(drive,folder.id,"masterdata.json",payload);
    ...
    if(action==="saveSapSnapshotInit"){
      ...
      let previous=null;
      if(oldManifestFile){try{previous=await readJson(drive,oldManifestFile.id);}catch(_e){}}
      ...
      const manifest={
        schemaVersion:3.2,storageMode:"chunked",
        duplicatePolicy:"replace_same_market_department_year_week",
        snapshotKey,revision,status:"uploading",uploadId:body.uploadId||null,
        market,department,targetYear:Number(s.year),targetWeek:Number(s.week),
        sourceFileName:body.sourceFileName||null,uploadedAt:body.uploadedAt||new Date().toISOString(),
        ...
        report:body.report||{},dedupe:body.dedupe||{},masterDataVersion:body.masterDataVersion||3,
        ...
        completedFactChunks:0,completedDecisionChunks:0
      };
      await upsertJson(drive,s.weekFolder.id,"lernbase.json",manifest);
      return res.status(200).json({ok:true,path:`${s.path}/lernbase.json`,snapshotKey,revision,replacedPreviousSnapshot:manifest.replacedPreviousSnapshot});
    }
    ...
    if(action==="saveSapDecisionChunk"){
      ...
      await upsertJson(drive,s.weekFolder.id,fileName,{schemaVersion:3.2,uploadId:body.uploadId||null,chunkIndex:idx,decisions});
      return res.status(200).json({ok:true,chunkIndex:idx,decisions:decisions.length,path:`${s.path}/${fileName}`});
    }
    ...
    if(action==="saveSapSnapshotFinalize"){
      ...
      if(factFiles.length!==expectedFacts) return res.status(409).json({error:`Speicherung unvollständig: ${factFiles.length} von ${expectedFacts} Datenblöcken vorhanden.`});
      if(decisionFiles.length!==expectedDecisions) return res.status(409).json({error:`Produktentscheidungen unvollständig: ${decisionFiles.length} von ${expectedDecisions} Blöcken vorhanden.`});
      ...
      await upsertJson(drive,s.weekFolder.id,"lernbase.json",manifest);
      return res.status(200).json({ok:true,path:`${s.path}/lernbase.json`,snapshotKey:manifest.snapshotKey,revision:manifest.revision,replacedPreviousSnapshot:Boolean(manifest.replacedPreviousSnapshot),rows:Number(manifest.totalRows)||0,chunks:factFiles.length});
    }
